package com.solo11come.api;

import com.solo11come.models.GeofencingRequest;
import com.solo11come.models.GeofencingResponse;
import com.solo11come.models.SubscriptionRequest;
import com.solo11come.models.SubscriptionResponse;
import retrofit2.Call;
import retrofit2.http.Body;
import retrofit2.http.Header;
import retrofit2.http.POST;

public interface DeviceStatusApi {
    @POST("device-reachability-status-subscriptions/v1/subscriptions")
    Call<SubscriptionResponse> createSubscription(
            @Header("x-rapidapi-key") String apiKey,
            @Header("x-rapidapi-host") String apiHost,
            @Header("x-correlator") String correlator,
            @Body SubscriptionRequest request
    );

    @POST("geofencing-subscriptions/v1/subscriptions")
    Call<GeofencingResponse> createGeofencingSubscription(
            @Header("x-rapidapi-key") String apiKey,
            @Header("x-rapidapi-host") String apiHost,
            @Body GeofencingRequest request
    );
}
